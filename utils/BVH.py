...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    # quat_rot = Quaternions.from_euler(np.radians(rotations), order=order, world=world) # works for world =True. for world=False do not reverse order and reverse rotations
    reorder = [order.index('x'), order.index('y'),
               order.index('z')]  # even if seq is not 'xyz', angles should be ordered by xyz
    quat_rot = Quaternions.from_euler(np.radians(rotations[:, :, reorder]), order=order,
                                      world=world)  # works for world =True. for world=False do not reverse order and reverse rotati
    ...
    if False:
        print('sanity check:')
        print('rotations from file (reordered as xyz): {}'.format(rotations[:, :, reorder][1, 0]))
        print('rotations from file (reordered as xyz): {}'.format(np.degrees(quat_rot.euler(order='xyz'))[1, 0]))

    return (Animation(quat_rot, positions, orients, offsets, parents), names, frametime)
                ...
                if i == 0:
                    ...
                    if positions or j == 0:
                        if save_global:
                            ...
                            f.write("%f %f %f %f %f %f " % (
                                0, 0, 0, rots[i, j, ordermap[order[0]]], rots[i, j, ordermap[order[1]]],
                                rots[i, j, ordermap[order[2]]]))
                    else:
                        f.write("%f %f %f " % (
                            rots[i, j, ordermap[order[0]]], rots[i, j, ordermap[order[1]]],
