...
#
N_JOINTS = 20
PARENTS = [-1, 0, 1, 2, 0, 4, 5, 0, 7, 8, 9, 10, 8, 12, 13, 14, 8, 16, 17, 18]
#
#
# class Animation:
#     """
...
#     return joints
#
# ### Original transforms_local function from Animation.py
...
#
#
# # this function should OVERIDE the function with same name in Animation.py
# # this function should OVERIDE the function with same name in Animation.py
# def transforms_local(anim):
#     """
#     Computes Animation Local Transforms
...
#     transforms = R.from_euler(angles=anim.rotations.euler(order='xyz').reshape(-1, 3), seq='xyz',
#                               degrees=False).as_matrix().reshape(
#         anim.shape + (3, 3))
#     transforms = np.concatenate([transforms, np.zeros(transforms.shape[:2] + (3, 1))], axis=-1)
#     transforms = np.concatenate([transforms, np.zeros(transforms.shape[:2] + (1, 4))], axis=-2)
#     transforms[:, :, 0:3, 3] = anim.positions
...
def offsets_from_bones(length,third_dimension=1):
    offsets = np.zeros((N_JOINTS, 3))
    offsets[0, 0] = 0  # Hips
    offsets[1, 0] = -length[0]  # Hip Right
    offsets[2, third_dimension] = -length[1]  # Knee Right
    offsets[3, third_dimension] = -length[2]  # Foot Right
    offsets[4, 0] = length[0]  # Hip Left
    offsets[5, third_dimension] = -length[1]  # Knee Left
    offsets[6, third_dimension] = -length[2]  # Foot left
    offsets[7, 0] = 0  # Hips 2nd Joint
    offsets[8, third_dimension] = length[3]  # Spine Middle
    offsets[9, third_dimension] = length[4]  # Neck
    offsets[10, third_dimension] = length[5]  # Nose
    offsets[11, third_dimension] = length[6]  # Head
    offsets[12, third_dimension] = length[4]  # Neck Left
    # offsets[12, 0] = 1
    offsets[13, third_dimension] = length[7]  # Shoulder Left
    offsets[14, third_dimension] = length[8]  # Elbow Left
    offsets[15, third_dimension] = length[9]  # Hand Left
    offsets[16, third_dimension] = length[4]  # Neck Right
    # offsets[16, 0] = -1
    offsets[17, third_dimension] = length[7]  # Shoulder Right
    offsets[18, third_dimension] = length[8]  # Elbow Right
    offsets[19, third_dimension] = length[9]  # Hand Right
    return offsets


# # this function should be added to Animation.py
...
#     return offsets
#
#     # root_hip_bone = 12.719393604653
#     # hip_knee_bone = 43.429100912261
#     # knee_foot_bone = 44.8767017425826
#     # root_spine_bone = 24.5913012437421
#     # spine_neck_bone = 24.8462964506057
#     # neck_nose_bone = 9.27524780924562
#     # nose_head_bone = 11.4999962169752
#     # neck_shoulder_bone = 12.4881979654096
#     # shoulder_elbow_bone = 25.9758046908119  # 25.9757993755427
#     # elbow_hand_bone = 24.5542024364365  # 24.5541985501975
#     # offsets = np.asarray([
#     #                       [0, 0, 0],
#     #                       [-root_hip_bone, 0, 0],
#     #                       [0, -hip_knee_bone, 0],
...
#     #                       [0, nose_head_bone, 0],
#     #                       [0, spine_neck_bone, 0],
#     #                       [0, neck_shoulder_bone, 0],
#     #                       [0, shoulder_elbow_bone, 0],
#     #                       [0, elbow_hand_bone, 0],
#     #                       [0, spine_neck_bone, 0],
#     #                       [0, neck_shoulder_bone, 0],
#     #                       [0, shoulder_elbow_bone, 0],
...
#
# # this function should be added to Animation.py
# def animation_from_positions(positions):
...
def load_from_network(translation, rotations, length, third_dimension=1, average_length=5, tanslation_scale=3):
    rotations = rotations.reshape((-1, N_JOINTS, 4))
    length = length[0]
    ...
    positions = np.expand_dims(translation, axis=1) # np.zeros(((len(rotations), N_JOINTS, 3)))

    PARENTS = [-1, 0, 1, 2, 0, 4, 5, 0, 7, 8, 9, 10, 8, 12, 13, 14, 8, 16, 17, 18]
    offsets = offsets_from_bones(length, third_dimension)
