...
from utils.myAnimation import Animation
...
def concat_rotations(rot1, rot2):
    q1 = rot1.as_quat()
    q2 = rot2.as_quat()
    rot = R.from_quat(np.append(q1, q2, axis=0))
    return rot
...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    for line in f:
        ...
        if rmatch:
            ...
            orients = concat_rotations(orients, R.identity(1))
        ...
        if fmatch:
            ...
            # rotations = R.identity(fnum * len(orients))
            continue
    ...
    reorder = [order.index('x'), order.index('y'), order.index('z')] # even if seq is not 'xyz', angles should be ordered by xyz
    # quat_rot = Quaternions.from_euler(np.radians(rotations[:,:,reorder]), order=order[::-1], world=world)
    # quat_rot = Quaternions.id(rotations.shape[:2])
    # quat_rot.qs =  R.from_euler(seq=order[::-1], angles=rotations.reshape(-1,3)[:,reorder], degrees=True).as_quat().reshape(rotations.shape[:2]+(4,))
    rotations_linear = rotations.reshape(-1,3)
    for i in np.arange(rotations_linear.shape[0]):
        if np.isnan(rotations_linear[i]).any():
            rotations_linear[i] = [0,0,0]
    rot_rot =  R.from_euler(seq=order[::-1], angles=rotations_linear[:,reorder], degrees=True)

    return (Animation(rot_rot, positions, orients, offsets, parents), names, frametime)
...
def save(filename, anim, names=None, frametime=1.0/24.0, order='zyx', positions=False, orients=True):
    ...
    with open(filename, 'w') as f:
        ...
        rots = anim.rotations.as_euler(seq=order[::-1], degrees=True).reshape((-1, anim.positions.shape[1], 3))
        poss = anim.positions
