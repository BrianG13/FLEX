...
def visual_sequence_result_without_image(pose_2d, pose_3d_predict, pose_3d_gt, save_path=None):
    ...
    for i in range(20):
        ...
        show2Dpose(pose_2d[i, :], ax0, radius=500)
        ax2.invert_yaxis()
...
def show3Dpose(channels, ax, radius=600, lcolor="#3498db", rcolor="#e74c3c", add_labels=True,save_path=None):  # blue, orange
    """
...
def show2Dpose(channels, ax, radius=600, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):
    ...
    for i in np.arange(len(I)):
        ...
        if np.mean(x) != 0 and np.mean(y) != 0:
            ax.plot(x, y, lw=7, c=lcolor if LR[i] else rcolor)
    ...
    ax.set_aspect('equal')
    # for i in range(20):
    #     ax.text(vals[i, 0],vals[i, 1], f'{i}', color='black')
